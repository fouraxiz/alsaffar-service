...
function isVideoFile(url: string): boolean {
  ...
  if (/\.(jpe?g|png|webp|gif|bmp)$/.test(clean)) return false;
  if (/\.(mp4|webm|mov|m4v|ogg)$/.test(clean)) return true;
  return /\/video\/?$/.test(clean) || /[?&]kind=video\b/.test(lower);
}
...
export default function CVDetailModal({ worker, onClose, onProceed }: Props) {
  ...
  const customerLoginUrl = portalAuthUrl(portalUrl, 'login', 'customer', returnUrl);
          ...
          <div className="flex flex-col md:flex-row h-full overflow-y-auto bg-white">
            ...
            <div className="w-full md:w-2/5 bg-gray-50 flex flex-col">
              <div className="relative aspect-[3/4] w-full bg-black">
                {isPlayingVideo && worker.videoUrl ? (
                  isVideoFile(worker.videoUrl) ? (
                    <video
                  ...
                  <>
                    ...
                    {/* Play uploaded introduction video / image when available */}
                    {worker.hasVideo && worker.videoUrl && (
                      <div
              ...
              <div className="p-4 md:p-6 mt-auto print:hidden">
                <button
                  type="button"
                  onClick={() => requireCustomerLogin(() => handleDownloadPDF())}
            ...
            <div className="w-full md:w-3/5 p-6 md:p-8 flex flex-col">
              ...
              <div className="mt-auto pt-4 border-t border-gray-100 flex flex-col sm:flex-row gap-3 print:hidden">
                <button
                  type="button"
                  onClick={() => requireCustomerLogin(onProceed)}
