    ...
    setMessages([
      {
        id: 'welcome',
        text: isAr 
          ? "مرحباً! أنا المساعد الذكي لمكتب الصفار للاستقدام. كيف يمكنني مساعدتك اليوم؟ يمكنك الاستفسار عن: العمالة المتوفرة، السير الذاتية، الأسعار، أو إجراءات الاستقدام."
          : "Hello! I am the AI assistant for Alsaffar Manpower Recruitment. How can I help you today? You can ask about: available workers, CV requests, prices, or recruitment procedures.",
        sender: 'bot',
        timestamp: new Date()
      }
    ]);
  ...
  return (
    <>
      ...
      <button 
        ...
        className={`fixed bottom-6 right-6 z-50 w-[60px] h-[60px] bg-[#0ea5e9] hover:bg-[#0284c7] text-white rounded-full flex items-center justify-center shadow-xl transition-all duration-300 ${isOpen ? 'scale-0 opacity-0 pointer-events-none' : 'scale-100 opacity-100'}`}
        aria-label="Open Chat"
      >
        <span className="font-bold text-[15px]">{t('chatToggle')}</span>
      </button>

      {/* Chat Window */}
      <div 
        className={`fixed bottom-6 right-6 z-50 w-[360px] max-w-[calc(100vw-32px)] bg-[#111827] rounded-xl shadow-2xl overflow-hidden transition-all duration-300 origin-bottom flex flex-col border border-gray-800 ${isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-4 pointer-events-none'}`}
        dir={isAr ? 'rtl' : 'ltr'}
      >
        {/* Header */}
        <div className="bg-[#111827] text-white p-5 pb-4 flex justify-between items-start border-b border-gray-800">
          <div>
            <h3 className="font-bold text-xl">{t('teamName')}</h3>
            <p className="text-sm text-gray-400 mt-1">{t('teamRole')}</p>
          </div>
          <button 
            onClick={() => setIsOpen(false)} 
            className="text-gray-400 hover:text-white border border-gray-700 rounded-md p-1 transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div 
          ref={chatBodyRef} 
          className="bg-[#0f172a] h-[280px] p-5 flex flex-col gap-4 overflow-y-auto"
        >
          {messages.map((msg) => {
            ...
            return (
              <div
                key={msg.id}
                className={`max-w-[85%] p-3.5 rounded-xl border text-[14px] leading-relaxed shadow-sm ${alignmentClass} ${roundedClass} ${bgClass}`}
              >
                {msg.text}
              </div>
            );
        ...
        <form 
          ...
          className="p-4 bg-[#111827] border-t border-gray-800"
        >
          <input 
            type="text"
            ...
            className="w-full bg-[#1f2937] text-white placeholder:text-gray-500 border border-gray-700 focus:border-[#25D366] focus:ring-1 focus:ring-[#25D366] rounded-lg px-4 py-2.5 text-[15px] outline-none transition-colors"
          />
        </form>
      </div>
    </>
